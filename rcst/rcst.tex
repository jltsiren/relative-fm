...
We have implemented the relative compressed suffix tree in C++, extending the old relative FM-index implementation.\footnote{The implementation is available at \url{https://github.com/jltsiren/relative-fm}.} The implementation is based on the \emph{Succinct Data Structure Library 2.0}~\cite{Gog2014b}. Some parts of the implementation have been parallelized using \emph{OpenMP} and the \emph{libstdc++ parallel mode}.
...
As our primary target sequence, we used the \emph{maternal haplotype} of the \emph{1000 Genomes Project individual NA12878}~\cite{Rozowsky2011}. As the target sequence, we used the 1000 Genomes Project version of the \emph{GRCh37 assembly} of the \emph{human reference genome}.\footnote{\url{ftp://ftp.1000genomes.ebi.ac.uk/vol1/ftp/technical/reference/}} Because NA12878 is female, we also created a reference sequence without chromosome Y.
...
Describe the FM-index and LCP reference. Describe sample periods. Tell why the period should be prime with suffix order sampling.

Table~\ref{table:construction}: Index construction time/space, LCS lengths. Note that basic RFM is much better optimized.
...
\textbf{Time} & \textbf{Space} \\
\hline
yes & 3096M & 3036M & 2992M & 2980M & 2.35 min & 4.96 GB & -- & -- & 379 min & 99.0 GB \\
no  & 3036M & 3036M & 2991M & 2980M & 2.28 min & 4.86 GB & -- & -- & 398 min & 97.2 GB \\
\hline
\end{tabular}
\end{center}
\end{table}

Table~\ref{table:indexes}: index sizes.
...
\hline
 &
\multicolumn{2}{c|}{\textbf{FM-index}} &
...
Table: component sizes (full RFM, RLCP)
