...
In a straightforward implementation, the \emph{phrase pointers} $p_{i}$ and the mismatching characters $c_{i}$ can be stored in arrays $A_{p}$ and $A_{c}$. These arrays take $z \log \abs{S}$ bits and $z \log \sigma$ bits, respectively. To support random access in the target sequence, we can encode phrase lengths as bitvector $B_{\ell}$ of length $\abs{S}$ \cite{Kuruppu2010}. We set $B_{\ell}[j] = 1$, if $S[j]$ is the first character of a phrase. The bitvector requires $z \log \frac{n}{z} + \Oh(z)$ bits, if we encode it using the \sdarray{} representation \cite{Okanohara2007}. To extract $S[j]$, we first determine the phrase $w_{i}$, with $i = \mrank_{1}(B_{\ell}, j)$. If $B_{\ell}[j+1] = 1$, we return the mismatching character $A_{c}[i]$. Otherwise we determine the phrase offset with a \select{} query, and return character $R[A_{p}[i] + j - \mselect_{1}(B_{\ell}, i)]$.

The \select{} query can be avoided by using \emph{relative pointers} instead of absolute pointers \cite{Ferrada2014}. By setting $A_{p}[i] = p_{i} - \mselect_{1}(B_{\ell}, i)$, the general case simplifies to $S[j] = R[A_{p}[i] + j]$. If most of the differences between the reference and the target sequence are single-character \emph{substitutions}, $p_{i+1}$ will often be $p_{i} + \ell_{i} + 1$. This corresponds to $A_{p}[i+1] = A_{p}[i]$ with relative pointers, making \emph{run-length encoding} the pointer array worthwhile.


\section{Relative FM-index}

The \emph{relative FM-index} (\RFM) \cite{Belazzougui2014} is the FM-index of target sequence $S$ relative to the FM-index of reference sequence $R$. It is based on finding (or approximating) the \emph{longest common subsequence} (\LCS) of $\mBWT(R)$ and $\mBWT(S)$.

\begin{itemize}
\item RFM: structures, rank queries, construction
