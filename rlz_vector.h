...
/*
  ...
  compress()/decompress() to switch between the encodings. Method
  serialize() will always write the plain bit_vector, while writeTo()
  can be used to write the compressed vector.

  FIXME: Instructions for loading the compressed variant.
  FIXME: Template
*/
class rlz_vector
{
...
public:
  ...
  size_type rank(size_type i) const
  {
    ...
    return rank_support_rlz_trait<t_b>::adjust_rank(this->m_v->plain_rank(i), this->m_v->size());
  }
